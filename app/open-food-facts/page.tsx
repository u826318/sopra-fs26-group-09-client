...
import type { ReceiptMatchedItem, ReceiptUploadSession } from "@/types/receipt";
...
function OpenFoodFactsPortalContent() {
  ...
  const receiptItems = useMemo<ReceiptMatchedItem[]>(() => {
    ...
    return receiptUploadSession.result.items.filter(isReceiptMatchedItem);
  ...
  useEffect(() => {
    ...
    const loadReceiptProductCards = async () => {
      ...
      for (let index = 0; index < receiptItems.length; index += 1) {
        ...
        const candidates = (item.productSearch?.candidates ?? []).slice(0, 5);
          ...
          <Space orientation="vertical" size="large" style={{ width: "100%" }}>
            ...
            {hasSearchNotice(nameSearchResponse) ? (
              <Alert
                ...
                showIcon
                message={nameSearchResponse?.message}
              />
            ) : null}
        ...
        >
          <Space orientation="vertical" size="large" style={{ width: "100%" }}>
            <Alert
              type="success"
              showIcon
              message={`Receipt scanned: ${receiptUploadSession?.result.merchantName ?? "Unknown merchant"}`}
                ...
                <div key={itemKey} className={styles.receiptLineBlock}>
                  ...
                  {hasSearchNotice(item.productSearch) ? (
                    <Alert
                      ...
                      showIcon
                      message={item.productSearch?.message}
